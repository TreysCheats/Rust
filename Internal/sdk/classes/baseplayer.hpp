...
inline class c_BasePlayer : c_prefab {
public:
	static enum Flags
	{
		Unused1 = 1,
	...
	c_item* getHeldItem() {
		int active_id = decrypt.decrypt_ulong(memory.read<uint64_t>(getAddress() + oBasePlayer::oActiveItemID));
		if (active_id == 0) return nullptr;
		uint64_t inventory = decrypt.decrypt_inventory(memory.read<uint64_t>(getAddress() + oBasePlayer::oInventory));
